...
export const ChatAssistant: React.FC<ChatAssistantProps> = ({ isIntroActive }) => {
  ...
  // When closed, render nothing. This removes the floating access pad from the lower right of the page.
  if (!isOpen) {
    return null;
  }
  ...
  return (
    <div className="fixed bottom-6 right-6 w-[22rem] sm:w-96 md:w-[28rem] max-h-[85vh] h-[620px] bg-white/95 backdrop-blur-xl rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3),0_0_0_1px_rgba(0,0,0,0.1)] flex flex-col z-[160] overflow-hidden ring-1 ring-white/60 flex-shrink-0 animate-in slide-in-from-bottom-8 fade-in duration-300 pointer-events-auto">
      {/* Header */}
      ...
      {/* Chat Thread */}
      <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 bg-slate-50/70 relative z-10">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex gap-3 max-w-[88%] ${msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
            ...
            <div className={`px-4 py-3 rounded-2xl text-sm ${msg.role === 'user' ? 'bg-slate-800 text-white rounded-tr-sm shadow-md shadow-slate-200' : 'bg-white text-slate-800 shadow-sm border border-slate-200/60 rounded-tl-sm'}`}>
              <div className={`markdown-body ${msg.role === 'user' ? 'text-white' : 'text-slate-800 prose prose-sm prose-slate max-w-none prose-p:leading-relaxed prose-headings:font-bold prose-headings:text-slate-900 prose-a:text-blue-600 prose-code:text-blue-800 prose-code:bg-blue-50 prose-code:px-1 prose-code:rounded'}`}>
                {msg.role === 'user' ? (
                  msg.parts[0].text
                ) : (
                  <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
              {quickPrompts.map((qp, i) => (
                <button
                  ...
                  className="text-left px-3 py-2 rounded-xl bg-white hover:bg-blue-50/80 border border-slate-200/80 hover:border-blue-300/80 text-slate-700 hover:text-blue-700 text-xs font-medium transition-all shadow-xs flex items-center justify-between group cursor-pointer"
                >
                  <span className="truncate">{qp.label}</span>
                  <Zap size={11} className="text-slate-400 group-hover:text-blue-600 shrink-0 ml-1 opacity-0 group-hover:opacity-100 transition-opacity" />
                </button>
              ))}
            </div>
          </div>
        ...
        {isLoading && (
          <div className="flex gap-3 max-w-[85%] animate-in fade-in duration-300">
            ...
            <div className="px-4 py-3 rounded-2xl bg-white border border-slate-200/60 rounded-tl-sm shadow-sm flex items-center gap-2">
              ...
              <span className="text-xs text-slate-500 font-medium">Analyzing synoptic patterns...</span>
            </div>
          </div>
        )}
      ...
      {/* Input */}
      <div className="p-3.5 sm:p-4 bg-white/70 backdrop-blur-md border-t border-slate-100 shrink-0">
        <div className="flex items-end gap-2 bg-white border border-slate-200/80 rounded-2xl p-1.5 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] focus-within:ring-4 focus-within:ring-blue-500/10 focus-within:border-blue-400 transition-all duration-300">
          <textarea
            value={input}
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={handleKeyDown}
            placeholder="Ask about monsoon phenomena, bias correction, or regimes..."
            className="w-full max-h-32 min-h-[44px] bg-transparent text-xs sm:text-sm resize-none p-2.5 sm:p-3 focus:outline-none text-slate-800 placeholder-slate-400 font-medium"
            rows={1}
          />
          <button
            onClick={() => handleSend()}
