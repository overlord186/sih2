...
export const generateForecastReport = async (
  ...
  try {
    ...
    autoTable(doc, {
      ...
      body: [
        ['Mean Abs. Error (MAE)', `${metrics.maeBaseline.toFixed(2)} mm`, `${metrics.maeCorrected.toFixed(2)} mm`, `-${((metrics.maeBaseline - metrics.maeCorrected)/metrics.maeBaseline * 100).toFixed(1)}%`],
        ['Root Mean Sq. Error', `${metrics.rmseBaseline.toFixed(2)} mm`, `${metrics.rmseCorrected.toFixed(2)} mm`, `-${((metrics.rmseBaseline - metrics.rmseCorrected)/metrics.rmseBaseline * 100).toFixed(1)}%`],
        ['Systematic Bias', `${metrics.biasBaseline.toFixed(2)} mm`, `${metrics.biasCorrected.toFixed(2)} mm`, 'N/A'],
        ['Pearson Correlation (R)', `${metrics.pearsonRaw.toFixed(3)}`, `${metrics.pearsonCorrected.toFixed(3)}`, `+${((metrics.pearsonCorrected - metrics.pearsonRaw)/metrics.pearsonRaw * 100).toFixed(1)}%`]
      ],
      theme: 'grid',
